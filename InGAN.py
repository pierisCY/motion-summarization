...
class InGAN:
    def __init__(self, conf):
        ...
        # Define input tensor
        self.input_tensor = torch.FloatTensor(1, 1, conf.input_crop_size, conf.input_crop_size).cuda()
        self.real_example = torch.FloatTensor(1, 1, conf.output_crop_size, conf.output_crop_size).cuda()
    ...
    def train_g(self):
        ...
        if self.cur_iter > self.conf.G_extra_inverse_train_start_iter:
            for _ in range(self.conf.G_extra_inverse_train):
                ...
                self.loss_G_inverse.backward(retain_graph=True)
                self.optimizer_G.step()
    ...
    def train_one_iter(self, cur_iter, input_tensor):
