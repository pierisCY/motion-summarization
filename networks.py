...
class GANLoss(nn.Module):
    ...
    def wgan_loss(self,prediction, target):
    ...
    def forward_D(self, disc, weight, real, fake, args):
        alpha = torch.rand((1,), device=args.device)
        interpolates = alpha * real + ((1 - alpha) * fake.detach())
        interpolates.requires_grad_(True)

        disc_interpolates = disc.forward(interpolates,weight)

        gradients = torch.autograd.grad(outputs=disc_interpolates, inputs=interpolates,
                                            grad_outputs=torch.ones_like(disc_interpolates),
                                            create_graph=True, retain_graph=True, only_inputs=True)[0]
        gradient_penalty = ((gradients.norm(2, dim=1) - 1) ** 2).mean()
        gradient_penalty_back = gradient_penalty * args.lambda_gp
        gradient_penalty_back.backward(retain_graph=True)
...
class Generator(nn.Module):
    ...
    def __init__(self, base_channels=64, n_blocks=6, n_downsampling=3, use_bias=True, skip_flag=True):
        ...
        self.entry_block = nn.Sequential(
            nn.ReflectionPad2d(3), nn.utils.spectral_norm(nn.Conv2d(1, base_channels, kernel_size=7, bias=use_bias)),
        ...
        self.final_block = nn.Sequential(nn.ReflectionPad2d(3), nn.Conv2d(base_channels, 1, kernel_size=7), nn.Tanh())
